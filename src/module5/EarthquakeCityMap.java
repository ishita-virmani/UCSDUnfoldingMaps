...
//			map = new UnfoldingMap(this, 200, 50, 650, 600, new OpenStreetMap.OpenStreetMapProvider());
	...
	public void mouseClicked() {
		...
		selectEqMarkerIfClicked();
		selectCityMarkerIfClicked();
	}

	private void selectEqMarkerIfClicked() {
		for (Marker m : quakeMarkers) {
			...
			if (marker.isInside(map, mouseX, mouseY) && lastClicked == null) {
				...
				for (Marker mHidden : quakeMarkers) {
					if (mHidden != lastClicked) {
						mHidden.setHidden(true);
					}
				}

				for (Marker c : cityMarkers) {
					if (c.getDistanceTo(marker.getLocation()) > marker.threatCircle()) {
						c.setHidden(true);
					}
				}
				return;
	...
	private void selectCityMarkerIfClicked() {
		...
		for (Marker c : cityMarkers) {
			if (c.isInside(map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) c;
				lastClicked.setClicked(true);

				for (Marker mHidden : cityMarkers) {
					if (mHidden != lastClicked) {
						mHidden.setHidden(true);
					}
				}

				for (Marker eqm : quakeMarkers) {
					EarthquakeMarker m = (EarthquakeMarker) eqm;
					if (c.getDistanceTo(m.getLocation()) > m.threatCircle()) {
						m.setHidden(true);
					}
				}
				return;
